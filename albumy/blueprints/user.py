...
from albumy.forms.user import EditProfileForm, UploadAvatarForm, CropAvatarForm, ChangePasswordForm, \
    NotificationSettingForm, PrivacySettingsForm, DeleteAccountForm, EditEmailForm
...
def follow(username):
    ...
    if current_user.receiver_follow:
...
def notification_setting():
    ...
    if form.validate_on_submit():
        current_user.receive_collect_notification = form.receiver_collect_notification.data
        current_user.receive_comment_notification = form.receiver_comment_notification.data
        current_user.receive_follow_notification = form.receiver_follow_notification.data
        db.session.commit()
        flash('Notification settings updated.', 'success')
        return redirect(url_for('.index', username=current_user.username))
    form.receiver_collect_notification.data = current_user.receive_collect_notification
    form.receiver_comment_notification.data = current_user.receive_comment_notification
    form.receiver_follow_notification.data = current_user.receive_follow_notification
    return render_template('user/settings/change_notification.html', form=form)
...
def upload_avatar():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Image upload, place crop.', 'success')
    flash_errors(form)
...
def privacy_setting():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('.index', username=current_user.username))
    form.public_collections = current_user.show_collections
    return render_template('user/settings/change_privacy.html', form=form)


@user_bp.route('/settings/account/delete')
def delete_account():
